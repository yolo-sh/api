...
import (
	...
	"github.com/yolo-sh/api/internal/envvars"
	"golang.org/x/oauth2"
)
...
func getDescriptionForGitHubErrorCode(errorCode string) string {
	...
	if errorCode == "application_suspended" {
		return "The Yolo application has been suspended. " +
			"Please open a new issue at: https://github.com/yolo-sh/cli/issues/new"
	}

	if errorCode == "redirect_uri_mismatch" {
		return "The Yolo application has been misconfigured. " +
			"Please open a new issue at: https://github.com/yolo-sh/cli/issues/new"
	}

	if errorCode == "access_denied" {
		return "You have chosen to not authorize the Yolo application."
	}
